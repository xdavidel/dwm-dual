...
void bstack(Monitor *m) {
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
    if (i < m->nmaster) {
      ...
      resize(c, sx, sy, sw / (n - MIN(n, m->nmaster)) - (2 * c->bw),
             sh - (2 * c->bw), 0);
      sx += WIDTH(c) + iv;
    }
  }
...
void centeredmaster(Monitor *m) {
  ...
  int oh, ov, ih, iv;
  int mn = 0, ln = 0,
      rn = 0; // number of clients in master, left and right area
  Client *c;
...
void deck(Monitor *m) {
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
    if (i < m->nmaster) {
      resize(c, mx, my, mw - (2 * c->bw), mh / MIN(n, m->nmaster) - (2 * c->bw),
             0);
      my += HEIGHT(c) + ih;
    } else {
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
    if (i < m->nmaster) {
      resize(c, mx, my, mw - (2 * c->bw),
             mh * (c->cfact / mfacts) - (2 * c->bw), 0);
      my += HEIGHT(c) + ih;
    } else {
      resize(c, sx, sy, sw - (2 * c->bw),
             sh * (c->cfact / sfacts) - (2 * c->bw), 0);
      sy += HEIGHT(c) + ih;
    }
}
