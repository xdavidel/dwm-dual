...
static void unhidebar(const Arg *arg);
...
void drawbar(Monitor *m) {
  ...
  if (m == selmon) { /* extra status is only drawn on selected monitor */
    drw_setscheme(drw, scheme[SchemeNorm]);
    drw_text(drw, 0, 0, mons->ww, bh, lrpad / 2, "",
             0);                    /* clear extrabar hack */
    sw = TEXTW(estext) - lrpad + 2; /* 2px right padding */
    drw_text(drw, mons->ww - sw, 0, sw, bh, 0, estext, 0);
  ...
  for (m = mons; m; m = m->next) {
    ...
    if (!m->extrabarwin) {
      m->extrabarwin = XCreateWindow(
          dpy, root, m->wx, m->eby, m->ww, bh, 0, depth, InputOutput, visual,
